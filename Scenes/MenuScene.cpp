...
MenuScene::MenuScene(sf::Vector2f screenSize, std::shared_ptr<Game> game, Services& services) : Scene(screenSize, game, services)
{
	...
	load_btn->setTexture(mServices.textureManager->getTexture(Texture::SPRITESHEET).get());
	...
	save_btn->setTexture(mServices.textureManager->getTexture(Texture::SPRITESHEET).get());
